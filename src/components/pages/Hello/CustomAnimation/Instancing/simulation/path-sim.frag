...
void main() {
  ...
  float r1 = 0.5 - rand(uv + .1);
  float r2 = 0.5 - rand(uv + .2);
  float r3 = 0.5 - rand(uv + .3);
  vec3 rand3 = vec3(
    r1,
    r2,
    r3
  ...
  vec3 np = nextPos;
  float fTime = time * 10.0;

  if (length(lastPos.xyz) < 0.00001) {
    ...
    nextPos = updatedPos * rotateY(time * 10.5);
